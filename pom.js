...
class FormulasGenerator {
    /**
     * 口算题生器核心类，负责生成完整的口算题
     * @param {Object} addattrs - 加法属性：
     * 例：{"carry": 1} ，其中carry为1表示加法有进位，2表示加法无进位，3表示随机进位。
     * @param {Object} subattrs - 减法属性
     * 例：{"abdication": 1} ，其中abdication为1表示减法有退位，2表示减法无退位，3表示随机退位。
     * @param {Object} multattrs - 乘法属性
     * @param {Object} divattrs - 除法属性
     * 例：{"remainder": 2} ，其中remainder为1表示除法有余数，2表示除法无余数的整除，3表示随机余数。
     * @param {number} step - 步数
     * 运算步数，1+1 为1步，1+1+1为2步。最对可以step=3，其余的步数都不支持，会报错。
     * @param {number} number - 生成的口算题数量
     * 需要生成的口算题数量。
     * @param {boolean} is_result - 是求运算结果还是求运算项
     * 0 求运算结果，1求运算项
     * @param {boolean} is_bracket - 是否包含括号
     * 0 不包含括号，1包含括号
     * @param {Array} multistep - 多步运算的数值范围
     * 例：[[0, 99], [0, 99], [0, 99], [0, 99], [1, 99]] ，其中[0, 99]表示0到99的整数范围，[1, 99]表示1到99的整数范围。
     * @param {Array} symbols - 运算符列表
     * 例：[[1],[2],[3,4]], 其中[1]表示加法，[2]表示减法，[3,4]表示乘法和除法。
     * @return {Array}
     * 生成的口算题数组
     */
    constructor(addattrs, subattrs, multattrs, divattrs, step, number, is_result, is_bracket, multistep, symbols) {
        
        this.addattrs = addattrs|| {"carry": 1};
        this.subattrs = subattrs || {"abdication": 1};
        this.multattrs = multattrs;
        this.divattrs = divattrs|| {"remainder":2};
        this.step = step || 1;
        this.is_result = is_result || 0;
        this.is_bracket = is_bracket || 0;
        this.number = number || 10;
        this.multistep = multistep || [[0, 99], [0, 99], [0, 99], [0, 99], [1, 99]];
        this.symbols = symbols || [[1,2],[1],[1]];
        this.__data_list = [];  // 生成的口算题
        if (this.step === undefined) {
            throw new Error("required param signum is missing or signum is None");
        }
        if (![1, 2, 3].includes(this.step)) {
